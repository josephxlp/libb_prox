...
def gen_label_by_threshold(dsm_path, dtm_path, mask_path, threshold=0.5):
    ...
    # this must should assing all np.nan to 2
...
def get_label_name(dsm_file,name):
    ...
    tilen = os.path.basename(dsm_file).split('_')[0] #str(dsm_file).split('\\')[-2]
    return  os.path.join(dirn, f'{tilen}_label_thresh_{name}.tif')

def gen_label_by_anomality_detection_nvidia():
    pass 

def gen_binclass_labels(dsm_path,pdem_path,ldem_path,pmask_path,lmask_path):
